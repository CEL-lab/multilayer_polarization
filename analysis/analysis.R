...
g_men <- graph_from_adjacency_matrix(df_mat,"undirected")
tkplot(g_men)
...
C <- character(length(V(g_men)$name))
# Iterate over V
for (i in seq_along(V(g_men)$name)) {
name <- V(g_men)$name[i]
...
V(g_men)$name[1]
V(g_men)$name[2]
match(V(g_men)$name[1],names(color))
match(V(g_men)$name[2],names(color))
color[25]
...
tkplot(g_men, vertex.label=V(g_men)$labels, vertex.color=color_vector)
